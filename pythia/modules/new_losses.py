...
@registry.register_loss("new_m4c_decoding_bce_with_mask")
class M4CDecodingBCEWithMaskLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        #print("raw loss:" + str(loss))
        loss /= 1e8
        #print("1e8 loss:" + str(loss))
        argmax_inds = scores.argmax(dim=-1)
        ratio = 1
        for single_argmax in argmax_inds:
            ratio *= min((len(single_argmax) - len(torch.unique(single_argmax))), 1) + 1
            print(ratio)
        loss *= ratio
        #print("mask loss:" + str(loss))
        return loss
